...
{
    public sealed class ADCredential : IDisposable, IProcessCredential
    {
        //private const string LDAP_DSE = "LDAP://{0}/RootDSE";

        //private NetworkCredential _netCreds;
        private bool _disposed;
        private UserIdentity _netCreds;
        ...
        public ADCredential(string domain, string userName, SecureString password)
        {
            ...
            _netCreds = new UserIdentity(userName, domain, password);
            this.AuthUserName = Combine(userName, domain);
        }

        private static string Combine(string user, string domain) => string.Format("{0}\\{1}", domain, user);
        public void Dispose()
        {
            if (_disposed)
                return;
